...
const serve = ({ port, api, prefix='/' }) => {
  ...
  router.get(routes.txs, async ctx => {
  ...
  router.get(routes.addresses, async ctx => {
  ...
  router.get(routes.balance, ctx => getBalance({ address: ctx.params.address }))
  router.get(routes.blockNumber, getBlockNumber)
  router.get(routes.processedBlockNumber, getProcessedBlockNumber)
  router.post(routes.sendSignedTransaction, ctx => sendSignedTransaction(ctx.body.tx))

  // Object.keys(api.web3).forEach(method => {
  //   router.get(routes.web3, ctx => api.web3[method](ctx.params))
  // })
