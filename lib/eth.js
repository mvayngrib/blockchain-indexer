...
class EthApi {
  constructor(web3) {
    ...
    ;[
      ...
      this[method] = (...args) => {
        ...
        // const promise = this.queue.add(() => this._tryWeb3Call(wrap(orig, args)))
        // if (this.queue.pending > 50) console.log('concurrency', this.queue.pending)
        // if (this.queue.size > 50) console.log('pending', this.queue.size)
        // return promise
      }
    })
  ...
  async sendSignedTransaction(hex) {
    const { web3 } = this
    return web3.sendSignedTransaction(hex)
  }
