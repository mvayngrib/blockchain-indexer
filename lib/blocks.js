...
class Blocks extends EventEmitter {
  ...
  async getNextBlocks({ maxBatchSize, confirmationHeight }) {
    ...
    if (bufferedHeight < confirmationHeight) {
      const targetBlock = this.savedBlockNumber + confirmationHeight
      const reason = `configured confirmationHeight: ${confirmationHeight}, batchSize: ${n}`
      this.logger.debug(`waiting for block ${targetBlock} (${reason})`)
      await this._waitForBlock(targetBlock)
    }

    let n = Math.min(this.latestBlockNumber - this.savedBlockNumber, maxBatchSize)
    n = Math.max(confirmationHeight, n)
