...
class BlockProcessor extends EventEmitter {
  constructor({ web3, state }) {
    ...
    this.web3 = web3
    this.state = state
    this.logger = createLogger('blockprocessor')
  ...
  async _loadLatestBlockNumber() {
    const n = await this.web3.getBlockNumber()
...
const createBlockProcessor = opts => new BlockProcessor(opts)

module.exports = {
  create: createBlockProcessor,
}
