...
const fromDB = db => {
  ...
  const updateAddresses = async addressMap => {
    ...
    return Promise.mapSeries(batches, async addrs => {
      const batch = flatMap(addrs, address => {
        ...
        const i = txsIn.map(tx => createPut({ ...tx, address, inbound: true }))
        const o = txsOut.map(tx => createPut({ ...tx, address, inbound: false }))
        return i.concat(o)
      })
  ...
  const getAddress = async (address, blockHeight=0) => {
    assertHex(address)
    const gte = serialize.addrToTxMapping({ address, blockNumber: blockHeight })
    ...
    const [i, o] = partition(keys.map(unserialize.addrToTxMapping), 'inbound')
    return {
      txsIn: i.map(stripAddress),
      txsOut: o.map(stripAddress),
    }
  }
