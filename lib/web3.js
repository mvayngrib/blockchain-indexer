...
const configureProvider = (web3, opts) => {
  const { httpPort, wsPort, logger, live } = opts
  if (!live) {
    web3.setProvider(new Web3.providers.HttpProvider(`http://localhost:${httpPort}`))
    return
  }

  const provider = new Web3.providers.WebsocketProvider(`ws://localhost:${wsPort}`, WS_OPTS)
  provider.on('end', err => {
